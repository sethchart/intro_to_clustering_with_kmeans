def k_means(X, Y, k=2):
    ...
    for pt in all_pts:
        dists = []
        #...calculate the distance from the each cluster center...
        for c_pt in pts: 
            dists.append(np.linalg.norm(np.array(pt) - np.array(c_pt)))
        #...assign which cluster center is the minimum distance away to the point...
        # The index becomes the cluster assignment, which is 0,1,2...k-1
        # dist_min is actually the index, which is the cluster assignment. 
